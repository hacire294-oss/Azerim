...
- After the user submits their phone number, the client calls `submit` on the `linkHandler`, passing in the phone number.
  - This will trigger a `LAYER_READY` event if the user is eligible for Layer (that is, they're in Plaid's system and Plaid has all of the data that you marked as required in your template). Otherwise, it will trigger a `LAYER_NOT_AVAILABLE` event.
- If the user is not eligible for layer, the client will ask the user for their date of birth, and then make a second `submit` call on the `linkHandler`, passing in the birthdate.
  - This will trigger a `LAYER_READY` event if the user is eligible for Extended Autofill (that is, Plaid is capable of retrieving all of the data that you marked as required in your template). Otherwise, it will trigger a `LAYER_AUTOFILL_NOT_AVAILABLE` event.
